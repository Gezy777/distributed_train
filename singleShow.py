    ...
    for epoch in range(epochs):
        ...
        for step in range(train_steps_per_epoch):
            ...
            if model_engine.is_last_stage():
                ...
                # swanlab.log({"train/loss_time": loss}, step=int((time.time() - start_time) * 10))
                swanlab.log({"A/train_loss": loss}, step=train_step)
            train_step += 1

        time_train = time.time()
        print(f"Epoch{epoch}的训练时间为{(time_train - time_last):.4f}")
        # swanlab.log({"train/epoch_time": time_train - time_last}, step=int((time.time() - start_time) * 10))
        swanlab.log({"A/epoch_time": time_train - time_last}, step=epoch)
        time_last = time.time()
